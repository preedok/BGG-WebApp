...
const getAnalytics = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.findAll({
    ...
    include: [
      ...
      { model: OrderItem, as: 'OrderItems', attributes: ['id', 'type', 'product_name', 'quantity', 'unit_price', 'subtotal'], required: false, include: [{ model: Product, as: 'Product', attributes: ['id', 'code', 'name', 'type'], required: false }] }
    ],
    order: [['created_at', 'DESC']],
    limit: 500
  });
...
const exportReportExcel = asyncHandler(async (req, res) => {
  ...
  const orders = await Order.findAll({
    ...
    include: [
      ...
      { model: OrderItem, as: 'OrderItems', attributes: ['type', 'product_name', 'quantity'], required: false }
    ],
    order: [['created_at', 'DESC']],
    limit: 2000
  });
