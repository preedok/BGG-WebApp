...
const STATUS_LABELS: Record<string, string> = {
  ...
  seat_reserved: 'Kursi Direservasi',
  booking: 'Booking',
...
const TicketDashboard: React.FC = () => {
  ...
  const handleExportExcel = async () => {
    setExporting(true);
    try {
      const res = await ticketApi.exportExcel();
      const blob = new Blob([res.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `rekap-tiket-${Date.now()}.xlsx`;
      a.click();
  ...
  const stats = [
    { title: 'Total Order (Tiket)', value: d.total_orders ?? 0, subtitle: 'Order dengan item tiket', icon: <Plane className="w-6 h-6" />, color: 'from-sky-500 to-blue-600' },
    { title: 'Total Item Tiket', value: d.total_ticket_items ?? 0, subtitle: 'Baris item tiket', icon: <Plane className="w-6 h-6" />, color: 'from-indigo-500 to-purple-500' },
    ...
    { title: 'Tiket Terbit', value: byStatus.ticket_issued ?? 0, subtitle: 'Sudah terbit', icon: <CheckCircle className="w-6 h-6" />, color: 'from-emerald-500 to-teal-500' }
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-start gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Rekapitulasi pekerjaan penerbitan tiket cabang Anda</p>
        </div>
          ...
          <div className="space-y-3">
            {pending.slice(0, 15).map((p: any) => (
              <div key={p.order_item_id} className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50 rounded-xl">
                ...
                <Button size="sm" onClick={() => navigate('/dashboard/tickets?order=' + p.order_id)}>
      ...
      {!data && !loading && (
        <Card>
          <p className="text-slate-600 text-center py-8">Belum ada order dengan item tiket di cabang Anda.</p>
        </Card>
      )}
    </div>
  );
