...
const OwnerDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      {/* Invoice & Order Saya - data owner */}
      <Card className="travel-card">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-stone-900">Invoice & Order Saya</h3>
            ...
            <div key={inv.id} className="p-4 sm:p-5 bg-stone-50 rounded-travel border border-stone-200">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                <div>
                  ...
                  <p className="text-sm text-stone-600">{inv.Order?.order_number ? `Order ${inv.Order.order_number}` : 'Order'}</p>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                ...
                return (
                  <div key={inv.id} className={`p-4 rounded-travel border-2 ${urgent ? 'bg-red-50 border-red-200' : 'bg-stone-50 border-stone-200'}`}>
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <p className="font-semibold text-stone-900 font-mono">{inv.invoice_number || inv.id}</p>
                        <p className="text-sm text-stone-600">{inv.Order?.order_number ? `Order ${inv.Order.order_number}` : ''}</p>
                      </div>
                      <Badge variant={getStatusBadge(inv.status)}>{INVOICE_STATUS_LABELS[inv.status] || inv.status}</Badge>
                    </div>
        ...
        <Card className="travel-card">
          ...
          <div className="space-y-4">
            <p className="text-stone-500 py-4 text-sm">Lihat invoice & order untuk detail perjalanan.</p>
          </div>
        </Card>
      </div>
    </div>
