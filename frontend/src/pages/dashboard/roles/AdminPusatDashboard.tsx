...
const AdminPusatDashboard: React.FC = () => {
  ...
  const getModalTitle = () => {
    if (modalType === 'status') return 'Daftar Order per Status';
    if (modalType === 'branch') return 'Daftar Order per Cabang';
    if (modalType === 'wilayah') return 'Daftar Order per Wilayah';
    if (modalType === 'provinsi') return 'Daftar Order per Provinsi';
    if (modalType === 'orders') return 'Semua Order';
        ...
        <>
              ...
              <div className="space-y-2 flex-1">
                {Object.entries(orders.by_status || {}).map(([status, count]) => (
                  <div key={status} className="flex justify-between items-center py-2 px-3 rounded-lg hover:bg-slate-50">
                    <Badge variant="info">{ORDER_STATUS_LABELS[status] || status}</Badge>
          ...
          <Card>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-slate-900">Order Terbaru</h3>
            ...
            {ordersRecent.length === 0 && <p className="text-slate-500 py-8 text-center">Belum ada order</p>}
          </Card>
