...
interface OrderItemRow  { id:string; type:ItemType; product_id:string; product_name:string; quantity:number; room_type?:RoomTypeId; room_breakdown?:HotelRoomLine[]; unit_price:number; }
...
const canManage = (role?:string) => role==='owner' || role==='invoice_koordinator';
...
const OrderFormPage: React.FC = () => {
  ...
  useEffect(()=>{
    ...
    ordersApi.getById(orderId)
      ...
      .catch(()=>{ showToast('Order tidak ditemukan','error'); navigate('/dashboard/orders-invoices?tab=orders'); })
      .finally(()=>setLoadingOrd(false));
  },[orderId,navigate,showToast]);

  useEffect(()=>{
    if(!orderId||!order?.OrderItems?.length||!products.length) return;
    const ois:any[]=order.OrderItems;
    ...
    for(const oi of ois){
      ...
      const t=(oi.type||'hotel') as ItemType;
      if(t==='hotel'&&oi.product_ref_id){
        if(!seen.has(oi.product_ref_id)){
          seen.add(oi.product_ref_id);
          const grp=ois.filter((o:any)=>o.type==='hotel'&&o.product_ref_id===oi.product_ref_id);
          rows.push({ id:oi.id||`row-${uid()}`, type:'hotel', product_id:oi.product_ref_id, product_name:oi.Product?.name||'',
            quantity:grp.reduce((s:number,o:any)=>s+(o.quantity||0),0),
            unit_price:disp(parseFloat(oi.unit_price)||0,oi.product_ref_id),
            room_breakdown:grp.map((o:any)=>{ const m=typeof o.meta==='object'?o.meta:{}; return{ id:o.id||`rl-${uid()}`, room_type:(m.room_type||o.room_type||'quad') as RoomTypeId, quantity:o.quantity??1, unit_price:disp(parseFloat(o.unit_price)||0,oi.product_ref_id), with_meal:m.with_meal??m.meal??false }; })
          });
        }
      } else {
        rows.push({ id:oi.id||`row-${uid()}`, type:t, product_id:oi.product_ref_id||'', product_name:oi.Product?.name||'', quantity:oi.quantity??1, room_type:(meta.room_type||oi.room_type) as RoomTypeId|undefined, unit_price:disp(parseFloat(oi.unit_price)||0,oi.product_ref_id||'') });
      }
    }
  ...
  const handleSubmit=(e:React.FormEvent)=>{
    ...
    for(const r of valid){
      if(r.type==='hotel'&&r.room_breakdown?.length){ for(const l of r.room_breakdown){ if(l.quantity<=0) continue; const meal=l.with_meal??false; payload.push({product_id:r.product_id,type:'hotel',product_ref_type:'product',quantity:l.quantity,unit_price:tIDR(l.unit_price,r),room_type:l.room_type,meal,meta:{room_type:l.room_type,with_meal:meal}}); } }
      else if(r.type==='hotel'&&r.room_type){ payload.push({product_id:r.product_id,type:'hotel',product_ref_type:'product',quantity:Math.max(1,r.quantity),unit_price:tIDR(r.unit_price,r),room_type:r.room_type,meta:{room_type:r.room_type}}); }
    ...
    if(isEdit&&orderId){
      ordersApi.update(orderId,{items:payload})
        .then(()=>{ showToast('Order diperbarui','success'); navigate('/dashboard/orders-invoices?tab=orders'); })
      ...
      ordersApi.create(body)
        .then(()=>{ showToast('Order dibuat','success'); navigate('/dashboard/orders-invoices',{state:{refreshList:true}}); })
        .catch((err:any)=>showToast(err.response?.data?.message||'Gagal membuat order','error'))
        .finally(()=>setSaving(false));
    }
  };
    ...
    <div className="ofp">
      ...
      <div className="ofp-hero">
        <div className="ofp-hero-inner">
          <button className="ofp-back" onClick={()=>navigate('/dashboard/orders-invoices?tab=orders')}>
            <ArrowLeft size={12}/> Kembali
          </button>
          <div className="ofp-hero-center">
            <div className="ofp-hero-eyebrow">{isEdit ? 'Edit Pesanan' : 'Pesanan Baru'}</div>
            <div className="ofp-hero-title">{isEdit ? 'Perbarui Order' : 'Buat Order Baru'}</div>
          </div>
      ...
      <div className="ofp-content">
        <form onSubmit={handleSubmit}>
              ...
              <div className="ofp-card-head">
                ...
                {canEditPrice&&(
                  <div className="ofp-flex-row" style={{gap:8}}>
                    <span style={{fontSize:10,color:'var(--tx3)',fontWeight:600,letterSpacing:'.07em',textTransform:'uppercase'}}>Input harga</span>
                    <div className="ofp-cur-sw">
                      {(['IDR','SAR','USD'] as const).map(c=>(
                        <button key={c} type="button" className={`ofp-cur-btn${priceCur===c?' on':''}`} onClick={()=>setPriceCur(c)}>{c}</button>
                      ))}
                    </div>
                  </div>
                )}
          ...
          <div className="ofp-footer">
            <button type="button" className="ofp-btn-ghost" onClick={()=>navigate('/dashboard/orders-invoices?tab=orders')}>Batal</button>
